...
class Setup:
    ...
    def __init__(self, sim_params, var, dim = None):
        if var[0] == 'a':
            ...
            if sim_params['BC'][dim]['type'] == 'periodic':
            ...
            if sim_params['BC'][var + dim]['type'] == 'periodic':
                self.N = self.Ngridpoints - 1
            else:
                self.N = self.Ngridpoints
            ...
            if sim_params['BC'][var]['type'] == 'periodic':
                self.N = self.Ngridpoints - 1
            else:
                self.N = self.Ngridpoints
...
def extract_active_grid(f_total_grid, sim_params):
    ...
    effect. The active grid, i.e. [z1.N, z2.N] is stored in
    sim_params['active_dims']

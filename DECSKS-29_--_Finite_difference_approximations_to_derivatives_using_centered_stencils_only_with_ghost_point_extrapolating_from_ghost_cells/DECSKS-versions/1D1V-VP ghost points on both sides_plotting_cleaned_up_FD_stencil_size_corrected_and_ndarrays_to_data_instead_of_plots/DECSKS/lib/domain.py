...
class Setup:
    ...
    def __init__(self, sim_params, var, dim = None):
        if var[0] == 'a':
            ...
            self.prepointmesh = np.array(np.outer( self.prepoints, np.ones([1, sim_params['total_dims'][1]]) ), dtype = int)
            ...
            self.prepointmesh = np.array(np.outer( np.ones([sim_params['total_dims'][0], 1]), self.prepoints), dtype = int)
            ...
            self.times = self.generate_Eulerian_mesh(self.N + 1) # includes time zero and self.N steps

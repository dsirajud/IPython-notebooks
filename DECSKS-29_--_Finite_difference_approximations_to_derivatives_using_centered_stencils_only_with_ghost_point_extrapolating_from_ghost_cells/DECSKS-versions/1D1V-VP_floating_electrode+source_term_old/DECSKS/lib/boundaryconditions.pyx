...
@cython.boundscheck(False)
def source_upper_boundary(np.ndarray[DTYPE_t, ndim=2] f_old,
                              np.ndarray[DTYPE_t, ndim=2] Uf_old,
                              np.ndarray[DTYPEINT_t, ndim=2] zpostpointmesh,
                              np.ndarray[DTYPEINT_t, ndim=2] vzpostpointmesh,
                              np.ndarray[DTYPEINT_t, ndim=2] vzprepointmesh,
    ...
    # README
    # this is a verbatim copy of the above routine absorbing_upper_boundary
    # the source is ad-hoc factored in in lib.split by tacking on the contributions as a += operation
    # and any outflowing particles are 'absorbed' (cutoff) as if it were an absorbing boundary condition.
...
def collector_lower_boundary(
    ...
    # generalize this later if desired, currently we ad-hoc add in the contribution from sources in lib.split.scheme
    # finally, check if an incoming source from the upper boundary is present. If so, factor in its contribution to the wall
    # charge in this time substep. Currently, we only have implemented velocity-dependent sources. Thus, since the sources
    # are not position or time dependent, we pre-compute the wall contribution before hand and add them on at each step.

    #    if sim_params['BC']['f']['upper'] == 'source':
    #        sim_params['sigma'][z.str]['lower'] += charge * sim_params['sources'][species]['N_lower']

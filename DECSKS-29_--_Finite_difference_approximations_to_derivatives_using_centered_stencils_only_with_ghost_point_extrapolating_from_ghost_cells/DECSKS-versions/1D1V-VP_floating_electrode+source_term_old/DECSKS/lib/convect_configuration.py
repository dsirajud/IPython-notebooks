...
def remap_step(
    ...
    f_k1, f_copy, Uf_copy, z = \
      eval(sim_params['distribution_function_boundarycondition_orchestrator_handle'][z.str])(
          f_k1, f_copy, Uf_copy, t, z, vz, dt, sim_params, charge, k = 0)
    ...
    f_k2, f_old, Uf_old, z = \
      eval(sim_params['distribution_function_boundarycondition_orchestrator_handle'][z.str])(
          f_k2, f_old, Uf_old, t, z, vz, dt, sim_params, charge, k = 1)
...
def finalize_density_periodic(sim_params, f_remapped, f_final, z, vz):
    ...
    # complete by periodicity for the last row, save the last column entry [x.Ngridpoints -1, :-1],
    # note that x.Ngridpoints - 1 = f_remapped.shape[0] = x.N
    f_final[f_remapped.shape[0],:-1] = f_remapped[0,:]

    # complete by periodicity for the last column, save the last row entry [:-1, vx.Ngridpoints-1]
    # note that vx.Ngridpoints - 1 = f_remapped.shape[0] = x.N
    f_final[:-1, f_remapped.shape[1]] = f_remapped[:,0]

    # complete column/row entry above that marks the overlap, i.e. the 'diagonal' entry [x.Ngridpoints-1, vx.Ngridpoints-1]
    # the completion can be three ways, i.e. it is equal to the density at [x.Ngridpoints-1, 0]
    # it is also equal to the density at [0, vx.Ngridpoints-1]
    # by dual periodicity (in x and vx) iti s then also equal to the density at [0,0]
    # note the gridpoint duple [x.Ngridpoints-1, vx.Ngridpoints-1] = [f_remapped.shape[0], f_remapped.shape[1]] = [x.N, vx.N]
    # since transpositions have been undone above

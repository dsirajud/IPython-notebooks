...
#         coefficients d to have GE of O(N), or in terms of LTE = GE + 1:
#
#         LTE[d] = O(N+1) for an Nth order method (global error)
#
#         d = dz ** dn * dnf, where dnf is the dn-th derivative of f, dn is the
#                             derivative number
#
#         Thus, for an Nth order CS scheme, this product needs to have LTE O(N+1)
#         which implies. Labeling the LTE on the derivative as p,
#
#         dn = 1, LTE(dnf) = p = O(N) so that LTE[dz * dnf] = O(N+1)
#         dn = 2, LTE(dnf) = p = O(N-1) so that LTE[dz**2 * dnf] = O(N+1)
#         dn = 3, LTE(dnf) = p = O(N-2) so that LTE[dz**3 * dnf] = O(N+1)
#         ... and so on
#
...
if __name__ == '__main__':
    ...
    for dn in range(1,N):
        ...
        p = N + 1 - dn   # LTE error p requirement decreases with each derivative
        ...
        single_dn = 'f' + str(dn)
        single_dn_outname = single_dn + '_LTE' + str(p) + outfilename_suffix

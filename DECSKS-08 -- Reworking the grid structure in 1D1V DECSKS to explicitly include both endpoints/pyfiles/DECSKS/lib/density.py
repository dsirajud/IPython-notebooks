...
def setup(sim_params, t, z1, z2 = None):
    ...
    if z2 is None:
        # init density container with dims len(z1.gridpoints) and zero time + t.N steps
...
def initial_profile(f0, sim_params, z1, z2 = None):
    ...
    if density == 'gaussian':
        mu, sigma = 0.0, 0.04
        return np.exp(-(z1.gridvalues - mu)**2/(2*sigma**2))
    ...
    if density == 'quint gaussian bell':
        ...
        f0[:] = 0.5*np.exp(-((z1.gridvalues + 0.25) / 0.03)**2) + 0.5*np.exp(-((z1.gridvalues + 0.2) / 0.03)**2) + np.exp(-((z1.gridvalues) / 0.06)**2) + 0.5*np.exp(-((z1.gridvalues - 0.2) / 0.03)**2) + 0.5*np.exp(-((z1.gridvalues - 0.25) / 0.03)**2)
        return f0
    ...
    if density == 'triple gaussian bell asymmetric':
        ...
        x_a, x_c = 0.25, -0.25
        f0[:]  = a * np.exp(-((z1.gridvalues + x_a) / w_a) ** 2) + np.exp(-((z1.gridvalues/w_b)**2)) + c*np.exp(-((z1.gridvalues + x_c)/w_c) ** 2)
        return f0

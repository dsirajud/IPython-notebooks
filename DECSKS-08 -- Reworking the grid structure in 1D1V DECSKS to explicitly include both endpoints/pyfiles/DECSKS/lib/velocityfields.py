...
class Velocity:
    ...
    """
    def __init__(self, field, z1, z2 = None):
        if z2 is None:
            self.N = z1.N
            self.gridpoints = np.array(range(self.N))
            #self.prepointvalues = call appropriate function
        ...
        if field.lower() == 'rotating':
            self.z1prepointvalues = -self.rotating(z2)
            self.z2prepointvalues = self.rotating(z1)

        else:
            self.prepointvalues = eval('self.' + field + '(z1.prepointvalues)')

    def rotating(self, z):
        return 2*np.pi*z.prepointvalues

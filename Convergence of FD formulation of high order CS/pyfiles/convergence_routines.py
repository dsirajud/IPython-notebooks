...
def df3(_x, a = 3/4., c = 1/2., w_a = 0.03, w_b = 0.06, w_c = 0.1, x_a = 0.25, x_c = -0.25):
    ...
    return a * (-2 / w_a**2) ** 2 * ( 3*(_x + x_a) - 2./w_a**2 * (_x + x_a) ** 3) * np.exp(-( (_x + x_a) / w_a) ** 2) \
      ...
      + c*(-2./w_c**2) * (3*(_x + x_c) - 2./w_c**2 * (_x + x_c)**3) * np.exp(-( (_x + x_c) / w_c) ** 2)

def df4(_x, a = 3/4., c = 1/2., w_a = 0.03, w_b = 0.06, w_c = 0.1, x_a = 0.25, x_c = -0.25):

    return a * (-2./w_a**2)**2 * (3 + 6*(-2./w_a**2)*(_x + x_a)**2 + (-2./w_a**2)**2 * (_x + x_a)**4) * np.exp(-( (_x + x_a)/w_a)**2) \
      + (-2./w_b**2) * (3 + 6*(-2./w_b**2)*_x ** 2 + (-2./w_b**2)**2 * _x**4) * np.exp(-( (_x/w_b)**2)) \
      + c*(-2./w_c**2) * (3 + 6*(-2./w_c**2) * (_x + x_c)**2 + (-2./w_c**2)*(_x + x_c)**4) * np.exp(-( (_x + x_c) / w_c) ** 2)
...
def FD_derivative_one_scheme_periodic(handedness = 'forward', asymmetry = 0, dn = 1, LTE = 3, Nx = 128):
    """the particular scheme: (handedness, asymmetry) is to be selected by inpsection of
    the tables generated by the FD scheme for a given order of derivative dn

            pyfiles/bin/generate_tables_of_finite_difference_schemes_for_a_given_LTE.py

    the following uses periodicity at the edges, hence it is necessary to use a function that is periodic
    over the domain [_a, _b]
    """
    x, dx, L = domain(_a = -1, _b = 1, _Nx = Nx)

    f = function(x)
    df = derivative(x, dn)

    # extract subdictionary pertaining to scheme of derivative order dn, and
    # corresponding weight and stencil lists
    dn_key = 'dn' + str(dn)
    FD_schemes = pyfiles.lib.make_FD_schemes_dict.store(dn, LTE)
    FD_schemes = FD_schemes[dn_key] # dictionary containing the family of
                                    # FD schemes or order dn
    ...
    w = FD_scheme['w']
    stencil = FD_scheme['stencil']
    stencil_size = len(stencil) # equivalently, _LTE + _dn

    # compute derivative approximations
    df_approx = np.zeros(Nx)
    for i in range(Nx):
        for s in range(stencil_size):
            df_approx[i] += w[s]*f[np.mod(i + stencil[s], Nx)]
    df_approx /= dx ** dn
...
def convergence_routine_single_scheme(NumGrids = 10, Nx = 18, LTE = 1, dn = 3, handedness = 'forward', asymmetry = 0):
    """executes FD_derivative_one_scheme_periodic(*args) on progressively finer grids.
    ...
    handedness -- (str) 'forward', 'backward', or 'central'
    asymmetry -- (int) degree of skewness from parent scheme's handedness

    NOTE: a derivative table must be generated for the required dn at chosen LTE
    by executing

    bin/$ python generate_tables_of_finite_difference_schemes_for_a_given_LTE.py LTE dn

    files will be stored and read in from pyfiles/etc/
    """
    ...
    error_norm[q] = FD_derivative_one_scheme_periodic(handedness, asymmetry, dn, LTE, Nx)
    grids[q] = Nx

    for q in range(1, NumGrids):
        Nx *= 2
        error_norm[q] = FD_derivative_one_scheme_periodic(handedness = 'forward', asymmetry = 0, dn = dn, LTE = LTE, Nx = Nx)
    ...
    plt.xlabel(r'Number of grid points $N_x$ in the domain $x\in [-1,1]$', fontsize = 16)
    ...
    plt.title(r'Error in $d^{%i}f/dx^{%i}$ using %s %i FD scheme at LTE = $O(\Delta x^{%i})$' % (dn,dn, handedness, asymmetry, LTE))
    plt.grid()

...
def finite_differences_periodic_BCs(f,z, sim_params):
...
def assemble_finite_difference_weight_matrices(sim_params, x):
...
def assemble_finite_difference_weight_matrix(sim_params, z):
    ...
    Wz -- (ndarray, ndim=3) Wz[dn, z.N, z.N] where each 2D matrix Wz[dn,:,:]
          ...
          for the phase space variable z
    """
    imax = z.N - 1
    ...
    for dn in range(1,sim_params['N']): # loop over all dn derivatives
        ...
        for i in range(z.N): # loop over each gridpoint (row of W matrix)
        ...
        Wz[dn,:,:] = W_dn
        # TODO figure out what coefficients are in the Wz matrix, I might have
        # TODO gotten confused and removed a normalization factor.

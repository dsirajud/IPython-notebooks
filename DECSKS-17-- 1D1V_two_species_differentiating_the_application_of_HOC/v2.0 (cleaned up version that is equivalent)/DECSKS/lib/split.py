    ...
    for s in range(len(stage)):
        ...
        if s == 0: # on first pass, the previous time step (n - 1) needs to be
            if coeff[s] == 'a': # advect x
                ...
                Ex = eval(sim_params['compute_electric_field_function_handle'])(fe, fi, x, vx, n-1, sim_params)
            ...
            if coeff[s] == 'a': # advect x
                ...
                # calculate electric field at most recent positions of ions and electrons
                # since not first time substep, pass time at n and store update at same n
                #                Ex = DECSKS.lib.fieldsolvers.compute_electric_field_fourier(fe, fi, x, vx, n, sim_params)

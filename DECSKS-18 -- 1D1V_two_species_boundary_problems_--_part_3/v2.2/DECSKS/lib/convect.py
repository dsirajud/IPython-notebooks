...
def remap_assignment(
    ...
    sim_params -- (dict) simulation parameters
    f_old -- (ndarray, dim=2) density from previous time step
    Uf -- (ndarray, dim=2) flux array
    z -- (instance) phase space variable being evolved
    vz -- (instance) generalized velocity of z
    charge -- (int) charge state, is set to +/-1 for hydrogenic plasmas
    index -- (str) 'nearest' (k1) or 'contiguous' (k2) determines the form of the
             remap assignment used. 'nearest' refers to the nearest
             grid point to the exact (non-integral) postpoint by rounding
             towards zero, thus 'contiguous' is the neighboring grid point
             along the same direction of travel. The density packet is allocated
             to each of these grid points in high order fractions U, which
             are stored as flux quantities Uf = U[i,j]*f[i,j] for all i,j.

    outputs:
    f_new -- (ndarray, dim=2) density with f_old remapped to
             according to the mapping

            [z.prepointmesh, vz.prepointmesh] --> [z.postpointmesh, vz.postpointmesh]

            where in this routine vz.postpointmesh = vz.prepointmesh as no boundary
            conditions that change the vz values have been implemented in this version.

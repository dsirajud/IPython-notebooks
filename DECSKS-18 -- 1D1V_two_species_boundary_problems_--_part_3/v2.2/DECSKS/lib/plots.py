...
class PlotSetup(Plots):
    ...
    def __call__(self, n):
        if len(self.f.shape) == 2:
            ...
            pylab.title('s18-XX DECSKS-2.2: $N_x$ = %d, $N_v$ = %d, $t^n$ = %2.3f, n = %03d' % (self.x.Ngridpoints, self.v.Ngridpoints, self.it*self.t.width, n))
            pylab.savefig(self.path + 's18-XX_' + self.filename)
            pylab.clf()
            return None

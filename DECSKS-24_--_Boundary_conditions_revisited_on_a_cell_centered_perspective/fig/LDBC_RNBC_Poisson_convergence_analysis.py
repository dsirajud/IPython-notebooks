...
if __name__ == '__main__':
    ...
    for grid in range(num_grids):
        ...
        phi = Poisson_solvers.LDBC_RNBC(n, Nx = Nx[grid], dx = dx, LBC = 3, RBC = 1.)
        phi_exact = 1/2. * x ** 2 - x + 3
        ...
        plt.plot(x,1+x, ':k', lw = 2, label = 'RNBC (slope = 1)')
        plt.grid()
        plt.axis([0,2, 2.45, 3])
        plt.legend(loc = 'upper left')
        grid_str = 'LDBC_RNBC_Nx_' + '%d' % Nx[grid] + '.png'

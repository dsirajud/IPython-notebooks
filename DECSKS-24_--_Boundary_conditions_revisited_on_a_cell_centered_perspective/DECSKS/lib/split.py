    ...
    for s in range(len(stage)):
        ...
        if coeff[s] == 'a': # advect x
            fe = DECSKS.lib.convect_configuration.scheme(
                    fe,
                    split_coeff*t.width, # fractional time substep
                    n,
                    int(stage[s]),
                    sim_params,
            ...
            print "after x advection"
            print "total mass of electrons"
            print np.sum(fe)
            ...
            print "after x advection"
            print "total mass of ions"
            print np.sum(fi)

            print ""
            print "after x advection"
            print "time substep s = %d" % int(stage[s])
            print "electron contribution"
            print np.sum(-fe) * x.width * vx.width

            print "ion contribution"
            print np.sum(fi) * x.width * vx.width

            IQ = np.sum(-fe + fi) * x.width * vx.width 

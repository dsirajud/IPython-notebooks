...
def compute_electric_field_fourier(fe, fi, x, vx, n, sim_params):
    ...
    # rule given out cell-centered grid
    n_total = np.sum(fi - fe, axis = 1) * vx.width
...
def extract_active_grid(sim_params, f_total_grid):
    ...
    # For a grid where gridpoints mark edges, not centers(!), we would compute
    #
    #     f_active_grid = f_total_grid[0:x.Ngridpoints-1, 0:vx.Ngridpoints-1]
    #
    # so that we evolve i = 0, 1, 2, ... , x.Ngridpoints - 2, and j = 0, 1, 2, ... , vx.Ngridpoints - 2
    # if both are periodic.

    # If one is not periodic, then all points are evolved ('active'), e.g.
    # if x is not periodic, we have
    #
    #     f_active_grid = f_total_grid[0:x.Ngridpoints, 0:vx.Ngridpoints-1]
    #

    # dummy assignment since method is deprecated
    f_active_grid = None

    ...
    for s in range(len(stage)):
        ...
        if s == 0: # on first pass, the previous time step (n - 1) needs to be
            if coeff[s] == 'a': # advect x
                ...
                E = eval(sim_params['compute_electric_field_function_handle'])(f, x, vx, n-1, sim_params)
                ax.prepointvaluemesh = -E
            ...
            if coeff[s] == 'a': # advect x
                ...
                E = eval(sim_params['compute_electric_field_function_handle'])(f, x, vx, n, sim_params)
                ax.prepointvaluemesh = -E

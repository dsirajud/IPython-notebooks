        ...
        if var in phasespace_vars:
            ...
            if HOC[var] == 'FOURIER':
                ...
                if BC['phi'][var]['type'] == 'BIAS_BIAS':
                    ...
                    if BC['f'][var]['lower'] != 'absorbing' or BC['f'][var]['upper'] != 'absorbing' or BC['f'][var]['lower'] != 'source' or BC['f'][var]['upper'] != 'source': 
                        raise InputError('A boundary condition on phi was specified as BIAS; however, the corresponding boundary condition on f is not compatible (must be set to absorbing or equivalent synonym)')
                    ...
                    if BC['f'][var]['lower'] != 'absorbing' or BC['f'][var]['lower'] != 'source': # all synonyms for 'absorbing' (except 'collector') have been seen by this point, and if encountered changed to 'absorbing'
                        raise InputError('A lower boundary condition on phi was specified as BIAS; however, the corresponding boundary condition on f is not compatible (must be set to absorbing or source)')

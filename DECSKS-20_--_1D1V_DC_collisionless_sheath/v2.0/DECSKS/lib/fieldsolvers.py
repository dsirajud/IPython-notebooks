...
def compute_electric_field_fd_dirichlet(fe, fi, x, vx, sim_params):

    phi = Poisson_DBC_1D1V_2S(fe, fi, x, vx, sim_params)
...
def compute_electric_field_fd_periodic(fe, fi, x, vx, sim_params):

    phi = Poisson_PBC_1D1V_2S(fe, fi, x, vx, sim_params)
...
def compute_electric_field_fourier_periodic(fe, fi, x, vx, sim_params):
...
def Gauss1D1V(ni, f, x, vx, sim_params):
    """Computes self-consistent electric field E by solving Gauss' law
    using FFT/IFFT.
...
def Gauss(ni, f, x, vx, sim_params):

...
def compute_electric_field_fourier(fe, fi, x, vx, n, sim_params):
    ...
    # here we solve for the field at each i = 0, 1, ..., x.Ngridpoints - 2
    # on the total grid, the endpoint i = x.Ngridpoints - 1 is assigned
    # at the end of the phase space solve by periodic boundary condition

    # integrate int dv_x (fi(x,v_x) - fe(x,v_x)) = sum (fi[i,j] - fe[i,j]) * vx.width
    # where we over j = 0, 1, 2, ... , vx.Ngridpoints - 2. Note that we do not
    # include the endpoint j = (vx.Ngridpoints - 1) in this Riemann sum as that
    # would overcount the velocity area under fi and fe at each i
    n_total = np.sum(fi[:x.Ngridpoints - 1, :vx.Ngridpoints-1]
                    - fe[:x.Ngridpoints - 1, :vx.Ngridpoints-1], axis = 1) * vx.width

    # To prepare for the transform, note that above we have also taken the open
    # period i = 0, 1, ..., x.Ngridpoints - 2, and not included the endpoint at
    # i = x.Ngridpoints - 1.

    # i.e. for the Fourier transform, we sample gridpoints over the open interval
    # [ax, bx) (i = 0, 1, ... , x.Ngridpoints - 2), we do not sample
    # over the closed period [ax, bx] (i = 0, 1, ... , x.Ngridpoints - 1)

    # It is well known that Fourier transforming a periodic function
    # should be done over the open period. For example, Fourier transforming
    # f(x) = cos(2x) over the interval [0, np.pi] will produce several nonzero
    # Fourier coefficients at all frequencies however, if we were to sample
    # over the open period [0, np.pi), then the only nonzero Fourier coefficient
    # would correspond to the wave number = +/- 2 * np.pi k / np.pi = +/- 2, i.e. k = 1.
    # i.e. a closed period isn't "seen" as periodic to a FFT, while an
    # open period is
    ...
    # extend for all [i,j], since x is periodic, extract the active grid points only
    E = np.outer(E[:x.N], np.ones([1, vx.N]))
...
def Poisson_6th_PBC(fe, fi,
    ...
    # for this periodic solver, the objects are constructed so that
    # the information of the endpoint is redundant, and hence only the
    # active grid in configuration is used, as usual,
    # i.e. i = 0, 1, 2, ..., x.Ngridpoints - 2. The endpoint is
    # assigned by periodicity at the conclusion of the entire phase
    # space solve.

    # Poisson eq. has -(charge density) = ne - ni, integrate over all
    # velocity, j = 0, 1, 2, ... , vx.Ngridpoints - 2. Note that
    # we do not include the endpoint j = vx.Ngridpoints - 1 in this
    # Riemann sum as that would overcount the area under the curve
    n_total = np.sum(fe[:x.Ngridpoints - 1, :vx.Ngridpoints - 1]
                     - fi[:x.Ngridpoints - 1, :vx.Ngridpoints - 1], axis = 1
                     ) * vx.width
    ...
    phi_avg = np.sum(phi[:x.Ngridpoints-1]) * x.width / x.L # note this does not include the right-end point as that would be
                                                            # overcounting area underneath the phi(x) curve
...
def Poisson_6th_LDBC_UDBC(fe, fi,
    ...
    phi -- (ndarray,dim=2) scalar potential, phi(x,v) = phi(x) at time t^n,
           for i = 0, 1, ... , x.N - 1, one full period
    """
    # x is not periodic, we index slice all entries
    # vx is being integrated over, as usual we integrate
    # using Riemann sums where the endpoint at j = vx.Ngridpoints-1
    # is not included as this would overcount the area
    n_total = np.sum(fe[:, :vx.Ngridpoints - 1]
                     - fi[:, :vx.Ngridpoints - 1], axis = 1
                     ) * vx.width
...
def Poisson_6th_LDBC_LNBC(fe, fi,
    ...
    phi -- (ndarray,dim=2) scalar potential, phi(x,v) = phi(x) at time t^n,
           for i = 0, 1, ... , x.N - 1, one full period
    """
    # x is not periodic, x.N == x.Ngridpoints, we index slice all entries
    # vx may or may not be periodic, vx.N != vx.Ngridpoints in general
    ...
    # the lower DBC sim_params['phi_BC'][x.str][0] = 0. will be kept at zero value as a reference potential (note: has been initialized as a zero vector, so already zero)
    ...
    print phi.shape
    # generate the 2D map for every [i,j], note that each row is constant
    # vx.N != vx.Ngridpoints in general
    phi = np.outer(phi, np.ones([1,vx.N]))
...
def Poisson_6th_UDBC_UNBC(fe, fi,
    ...
    phi -- (ndarray,dim=2) scalar potential, phi(x,v) = phi(x) at time t^n,
           for i = 0, 1, ... , x.N - 1, one full period
    """
    # the "direction" of integration does not matter, we integrate from
    # left to right by summing over velocity gridpoints
    # j = 0, 1, 2, ..., vx.Ngridpoints - 1

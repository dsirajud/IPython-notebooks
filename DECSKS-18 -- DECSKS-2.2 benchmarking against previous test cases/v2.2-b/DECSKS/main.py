...
ne_avg = np.sum(fe[:x.Ngridpoints - 1, :vx.Ngridpoints - 1]) * x.width * vx.width / x.L
print "ne_avg = %g" % ne_avg
...
print "integral (fe - fi) dx dvx = %g" % (np.sum(-fe[:x.Ngridpoints-1,:vx.Ngridpoints-1]
                                                 + fi[:x.Ngridpoints-1,:vx.Ngridpoints-1]
                                                 ) * x.width * vx.width)

print "integral 2 (f dx dvx - fi dx dvx) = %g" % ((np.sum(-fe[:x.Ngridpoints-1,:vx.Ngridpoints-1] + fi[:x.Ngridpoints-1,:vx.Ngridpoints-1] ) ) * x.width * vx.width)

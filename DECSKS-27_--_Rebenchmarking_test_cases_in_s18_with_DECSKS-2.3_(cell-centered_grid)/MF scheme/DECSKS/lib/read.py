...
def inputfile(filename):
   ...
   # --------------------------------------------------------------------------
    ...
    if BC['f']['x']['type'] == 'periodic':
        Nx_active = Nx - 1
    else:
        Nx_active = Nx

    if BC['f']['vx']['type'] == 'periodic':
        Nvx_active = Nvx - 1
    else:
        Nvx_active = Nvx
    ...
    for var in phasespace_vars:
        if BC['f'][var]['lower'] != 'periodic' and BC['f'][var]['upper'] != 'periodic':
            if split_scheme in ['LF2']:
                ...
                print "\nThe chosen split scheme involves negative time substeps, while the boundary conditions are non-periodic. The BOUNDED Vlasov-Poisson problem is irreversible. A split scheme with negative time substeps can only be used in periodic systems, which correspond to systems of infinite extent\n"
    ...
    for var in phasespace_vars:
        if HOC[var] == 'FD':
            ...
            # is passed for the generalized velocity Nvz
            ...
            if var[0] == 'v':
                ...
                Nvv = eval('N' + var[1] + '_active') # e.g. var = 'vx', var[1] = 'x',
                                         # so vvx is ax, whic has Nx gridpoints
                ...
                Nvv = eval('Nv' + var + '_active') # e.g. var = 'vx', var[1] = 'x', ax has Nx gridpoints
    ...
    phi_BC = {}
        # keys: 'x', 'y', 'z'
        # values: ndarrays of size eval('N' + var)
    ...
    if HOC['x'] == 'FD':
        ...
        phi_BC['x'] = None      # assigned as None so that lib.read return has enough values to unpack
    ...
    if HOC['y'] == 'FD':
        ...
        phi_BC['y'] = None      # assigned as None so that lib.read return has enough values to unpack
    ...
    if HOC['z'] == 'FD':
        ...
        phi_BC['z'] = None      # assigned as None so that lib.read return has enough values to unpack
    ...
    # See notebook s24 (Poisson solvers for DBC, NBC, SBC, and PBC boundary conditions)
    #
    #    DBC = Dirichlet boundary condition
    #    NBC = Neumann   boundary condition
    #    SBC = symmetry  boundary condition
    #    PBC = periodic  boundary condition

    # Each solver has its differencing matrix D on phi and its differencing
    # matrix B on the density n stored below in this routine, i.e. in the
    # lib.read.assemble_Poisson_6th_order_FD_solver_matrices method

    # the remaining differences are captured in an auxillary 1D vector phi_BC
    # whose values depend on the boundary conditions selected in
    # params_boundaryconditions.dat

    # phi_BC is a vector where only phi_BC[0] and phi_BC[-1] are nonzero
    # We make the following summarizing conclusions from the derivation
    # presented in the notebook s24:

    ##  DBCs

    #   will have an entry with a factor of -2, e.g phi_BC[0] = -2 * LBC
    #   where LBC is a lower boundary condition read in from the input file params.dat.
    #   Thus, we store the boundary values below for DBC cases with this factor of -2

    ## NBCs

    #   will have an entry with a factor of the cell width, z.width,
    #   e.g. phi_BC[0] = z.width * LBC
    #   where LBC is a lower boundary condition that is either zero (symmetry)
    #   or it depends on a total wall charge sigma whose value depends on time.
    #   Thus, we choose to assign these values inside lib.fieldsolver at
    #   each time step

    ## PBCs

    #   dose not have any boundary values, hence phi_BC = zeros(Nz)
    #   since the phi_BC vector has already been initialized to zeros below,
    #   no further action is required here if PBCs are selected.
        ...
        if var in phasespace_vars:
            ...
            if HOC[var] == 'FOURIER':
                ...
                if BC['phi'][var]['type'] == 'BIAS_BIAS':
                    ...
                    phi_BC[var][0] = -2 * float(Bias[var]['lower'])
                    # Dirichlet condition, phi = BIAS value
                    phi_BC[var][-1] = 2 * float(Bias[var]['upper'])
                    ...
                    raise NotImplementedError('at line 899 of lib.read, not implemented')
                    ...
                    #phi_BC[var][0] = var.width * (-sim_params['sigma'][var]['lower']), changes with time step

                    # Dirichlet condition, phi = -2 * BIAS value
                    phi_BC[var][-1] = -2 * float(Bias[var]['upper'])
                    ...
                    raise NotImplementedError('at line 985 of lib.read, not implemented')
                    ...
                    raise NotImplementedError('at line 1009 of lib.read, not implemented')
...
def output_files(filepath):
    ...
    filename_IQ = lines[16][lines[16].find(':')+1:].strip()
    ...
    filepath_IQ = rel_path + filename_IQ
...
def assemble_spectral_derivative_operator(Xi, xi,
    ...
    Xi -- (dict) to contain key/values:
        ...
        in general contains previous keys assigned by previous calls
    ...
    Nvz --  (int) total number of active gridpoints for vz
    ...
    zwidth = Lz / Nz_active # needed for 3D object, Xi = "(j zwidth xi)**q"
...
def assemble_Poisson_6th_order_FD_solver_matrices(Nx, BC):
    """
        ...
        Inputs:
        Nx -- (int) the number of grid points in a configuration variable x
    ...
    if BC['phi']['x']['type'] == 'PBC':
        ...
        raise NotImplementedError('While specifying LDBC and UNBC boundary conditions on the electric potential is accepted as valid input by DECSKS, it is on a TODO list and has not been implemented. For the time being, please revise your simulation to accommodate the desired circumstance. For example, the user might consider mirroring what they had intended so that an LNBC and UDBC are used, which is available in DECSKS')


    elif BC['phi']['x']['type'] == 'LDBC_LNBC':

        raise NotImplementedError('While specifying LDBC and LNBC boundary conditions on the electric potential is accepted (e.g. perhaps charge collecting walls were specified in params_boundaryconditions.dat, these are pure NBC-NBC conditions which are casted as equivalent Cauchy data at the lower boundary, i.e. LDBC/LNBC) by DECSKS, it is on a TODO list. We are getting on this as soon as possible as it is an important boundary setup.')


    elif BC['phi']['x']['type'] == 'UDBC_UNBC':

        raise NotImplementedError('While specifying UDBC and UNBC boundary conditions on the electric potential is accepted as valid input by DECSKS, it is on a TODO list and has not been implemented.')


    # all boundary conditions use the same B matrix,
    # i.e.
    #
    # $$\partial_x^2\phi = n \, \rightarrow \, \underline{\underline{D}}\cdot \underline{\phi} = (\Delta x)^2 \underline{\underline{B}}\cdot \underline}{n}
    #                                                                                               + \underline{\phi}_{BC}$$

...
def scheme(
    ...
    f_initial = f_initial[:z.N, :vz.N] # z = x, vz = vx
...
def remap_step(
    ...
    #    print "=========="
    #    print "BEFORE remap, inside x advection"
    #    print "total mass of f_alpha"
    #    print np.sum(f_old)
    #    print ""
    #    print "f_alpha contribution, abs value"
    #    print np.sum(f_old) * z.width * vz.width
    #    print "=========="

    f_copy =  f_old.copy()
    Uf_copy = Uf_old.copy()

    # k1: nearest grid point to true postpoint
    ...
    f_k1, f_copy, Uf_copy, z = \
      eval(sim_params['distribution_function_boundarycondition_orchestrator_handle'][z.str])(
          f_k1, f_copy, Uf_copy, dt, z, vz, sim_params, charge, k = 0)
    #    print "after BC"
    #    print f_k1[0,:]
    f_k1 += DECSKS.lib.remap.nearest_gridpoint_assignment(f_copy, Uf_copy, z.postpointmesh[0,:,:], vz.postpointmesh[0,:,:], z.N, vz.N)
    #    print "after remap assignment"
    #    for i in range(f_k1.shape[0]):
    #        for j in range(f_k1.shape[1]):
    #            print "f_k1 = %g" % f_k1[i,j]
    #            print "vz = %g" % vz.prepointvaluemesh[i,j]



    # k2: contiguous grid point to the nearest grid point along same direction of travel
    f_k2 = np.zeros_like(f_old)
    f_k2, f_old, Uf_old, z = \
      eval(sim_params['distribution_function_boundarycondition_orchestrator_handle'][z.str])(
          f_k2, f_old, Uf_old, dt, z, vz, sim_params, charge, k = 1)
...
def flux_limiter(
    ...
    # of less storage for local copies of the same arrays). Uf and f_old are ndarrays (not masked arrays)
    ...
    # combine masked data that has been limited by the operations above into a final Uf object to return.

    # the mask on Uf_neg masks out all positive values, hence negative values are unmasked (wherever mask == False)

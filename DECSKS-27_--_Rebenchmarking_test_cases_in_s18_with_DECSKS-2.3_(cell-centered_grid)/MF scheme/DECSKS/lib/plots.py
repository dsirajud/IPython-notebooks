...
class PlotSetup(Plots):
    ...
    def __call__(self, n):
        if len(self.f.shape) == 2:
            ...
            pylab.title('s17-03 DECSKS-2.3: $N_x$ = %d, $N_v$ = %d, $t^n$ = %2.3f, n = %03d' % (self.x.Ngridpoints, self.v.Ngridpoints, self.it*self.t.width, n))
            pylab.savefig(self.path + 's17-03_' + self.filename)
            pylab.clf()
            return None

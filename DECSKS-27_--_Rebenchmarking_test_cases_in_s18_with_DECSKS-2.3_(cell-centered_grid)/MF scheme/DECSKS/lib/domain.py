...
class Setup:
    ...
    def __init__(self, sim_params, var, dim = None):
        if var[0] == 'a':
            ...
            # if periodic, self.N = self.Ngridpoints - 1
            ...
            self.prepointmesh = np.array(np.outer( np.arange(self.N), np.ones([1, sim_params['active_dims'][1]]) ), dtype = int)
            ...
            if sim_params['BC']['f'][self.str]['type'] == 'periodic':
                self.N = self.Ngridpoints - 1
            else:
                self.N = self.Ngridpoints
            ...
            w, dw = np.linspace(self.a, self.b, num = self.Ngridpoints, retstep = True, endpoint = True)
            self.prepointvalues = w[:self.N]
            self.width = dw
            ...
            self.prepointmesh = np.array(np.outer( np.ones([sim_params['active_dims'][0], 1]), np.arange(self.N)), dtype = int)

            # for plots
            self.gridpoints = np.array(range(self.Ngridpoints))
            self.gridvalues = w # computed above
            ...
            # for plots
            self.gridpoints = np.array(range(self.Ngridpoints))
            self.gridvalues = w # full grid computed above

...
class Setup:
    ...
    def __init__(self, sim_params, var, dim = None):
        if var[0] == 'a':
            ...
            # TODO
            # the following two quantities are redundant in a cell-centered grid.
            # choose one at some point and remove all traces of the other
            self.Ngridpoints = sim_params['N' + dim] # Nx, Ny, or Nz
            self.N = sim_params['N' + dim] # Nx, Ny, or Nz TODO

            # to be filled at each time step with corresponding accelerations
            self.prepointvaluemesh = np.zeros(sim_params['total_dims'])

            # set up 3D postpointmesh container, shape = (2, dim1, dim2)
            # where in 1D1V, dim1 and dim2 are usually Nx and Nvx, and
            # the first index gives the postpoint k1 or k2
            # whatever the dims are, they are stored in sim_params['total_dims']
            # and are assembled in lib.read

            # assemble the 3-tuple that give its dimensions
            postpointmesh_dims = [2]
            for dim in sim_params['total_dims']:
                postpointmesh_dims.append(dim)

            # note: self.postpointmesh is a secret agent object, its a stand-in for the configuration postpointmesh during a
            # velocity remap assignment routine (lib.remap.assignment) which uses, e.g., the maps vx.postpointmesh and ax.postpointmesh
            # where ax.postpointmesh corresponds to just the transposed prepointmesh in configuration. This should be revised at some point
            self.postpointmesh = np.zeros(postpointmesh_dims, dtype = int) # container, to be filled at each timestep with postpoint index map
            self.prepointmesh = np.array(np.outer( np.arange(self.N), np.ones([1, sim_params['total_dims'][1]]) ), dtype = int)
            ...
            # create a cell-cented mesh
            self.prepointvalues, self.width = np.linspace(self.a, self.b, num = self.N, endpoint = False, retstep = True)
            self.prepointvalues += 0.5*self.width
            ...
            self.prepointvaluemesh = np.outer( np.ones([sim_params['total_dims'][0], 1]), self.prepointvalues)
            self.prepointmesh = np.array(np.outer( np.ones([sim_params['total_dims'][0], 1]), np.arange(self.N)), dtype = int)

            # for plots
            self.gridpoints = np.array(range(self.N))
            self.gridvalues = self.prepointvalues.copy() # TODO this used to be not redundant, i.e. this would show the total grid values
                                                         # where prepointvalues would show the active grid values. This object
                                                         # is likely tied to a plotting routine, but only one of these aforementioned
                                                         # objects are needed
            ...
            self.prepointvalues, self.width = np.linspace(self.a, self.b, num = self.N, retstep = True, endpoint = False)
            self.prepointvalues += 0.5*self.width
            ...
            self.prepointvaluemesh = np.outer( self.prepointvalues, np.ones([1, sim_params['total_dims'][1]] ) )
            self.prepointmesh = np.array(np.outer( np.arange(self.N), np.ones([1, sim_params['total_dims'][1]]) ), dtype = int)

            # for plots
            self.gridpoints = np.array(range(self.N))
            self.gridvalues = self.prepointvalues.copy()
            ...
            # TODO redundant objects
            self.N = sim_params['N' + var] # number of timesteps
            self.Ngridpoints = sim_params['N' + var] # number of timesteps
            ...
            self.times = self.generate_times(self.N + 1)
            self.str = var

    def generate_times(self, Num):
        """Mesh generator for times"""

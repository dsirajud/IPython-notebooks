...
def total_charge(f, fi, n, x, v, sim_params):
    ...
    IQ = np.sum(-f+ fi) * x.width * v.width 
...
def L1(f,n,x,v):
    ...
    return np.sum(f) * x.width * v.width
...
def L2(f,n,x,v):
    ...
    return np.sqrt(np.sum(f**2) * x.width * v.width)
...
def total_energy(f, fi, n, x, v, E, mu):
    ...
    return 1/2.*np.sum(f * v.prepointvaluemesh ** 2) * x.width * v.width \
      + 1/2. * mu * np.sum(fi * v.prepointvaluemesh ** 2 ) * x.width * v.width  \
...
def entropy(f,n,x,v):
    ...
    return -np.sum(f * np.log(f + eps)) * x.width * v.width

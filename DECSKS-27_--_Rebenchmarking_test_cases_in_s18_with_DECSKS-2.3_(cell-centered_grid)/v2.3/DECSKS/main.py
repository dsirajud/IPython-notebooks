...
sim_params = DECSKS.lib.read.inputfile('./etc/params_s27-01.dat')
...
print "ions at the edge"
print fi[0,:]

print "electrons at the edge"
print fe[0,:]
...
matplotlib.pyplot.title(r's27-01 Potential $\phi (x)$: $N_x$ = %d, $N_v$ = %d, $t^n$ = %2.3f, n = %04d' % (sim_params['total_dims'][0], sim_params['total_dims'][1], 0.0, 0))
it_str = 'it%05d' % 0
matplotlib.pyplot.axis([-10,10,0, 10.2])
matplotlib.pyplot.savefig('./plots/' + 'phi_s27-01_' + it_str)
...
for n in t.stepnumbers:
    ...
    matplotlib.pyplot.xlabel(r'position $x$', fontsize = 18)
    matplotlib.pyplot.ylabel(r'$\phi (t^n,x)$', fontsize = 18)
    matplotlib.pyplot.title(r's27-01 Potential $\phi (x)$: $N_x$ = %d, $N_v$ = %d, $t^n$ = %2.3f, n = %04d' % (sim_params['total_dims'][0], sim_params['total_dims'][1], t.stepnumbers[n]*t.width, n))
    it_str = 'it%05d' % n
    #    matplotlib.pyplot.axis([-10,10,-66,62])
    matplotlib.pyplot.savefig('./plots/' + 'phi_s27-01_' + it_str)
    matplotlib.pyplot.clf()

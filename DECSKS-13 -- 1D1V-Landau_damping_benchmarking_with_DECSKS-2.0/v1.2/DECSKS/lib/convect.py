...
class CourantNumber:
    ...
    def __init__(self, z):
        ...
        if self.numbers.all >= 0:
            self.int = np.array(np.floor(self.numbers), dtype = int)
        else:
            self.int = np.array(np.ceil(self.numbers), dtype = int)
...
def remap_rule(
    ...
    # ad hoc check for local density conservation
    #    if k1 is not None and k2 is not None:
    #        if f_old[ z.prepoints[i] ] != (f_remapped_MC_container[k1] + f_remapped_MC_container[k2]):
    #            print "density not conserved from advecting index %d, the difference is %g" % (z.prepoints[i],
    #                                                                                           (f_old[ z.prepoints[i] ] - (f_remapped_MC_container[k1] + f_remapped_MC_container[k2])))
    #    else:
    #        if f_old[ z.prepoints[i] ] != (f_remapped_MC_container[z.postpoints[i]]):
    #            print "density not conserved from advecting index %d, the difference is %g" % (z.prepoints[i],
    #                                                                                           (f_old[ z.prepoints[i] ] - f_remapped_MC_container[z.postpoints[i]]))
